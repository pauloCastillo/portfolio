...
def read_posts(db: db_depends, service: service_dep, current_user: current_user_dep):
...
def update_post(post_id: int, post: PostUpdate, db: db_depends, service: service_dep, current_user: current_user_dep):
    """Actualizar post existente."""
    return service.update(db, post_id, post)


@router.delete("/{post_id}", name="delete_post", status_code=status.HTTP_204_NO_CONTENT)
def delete_post(post_id: int, db: db_depends, service: service_dep, current_user: current_user_dep):
    """Eliminar post."""
